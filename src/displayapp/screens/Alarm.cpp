...
void Alarm::ShowInfo() {
  ...
  if (alarmController.State() == AlarmController::AlarmState::Set) {
    ...
    lv_label_set_text_fmt(
      txtMessage, "Cas do\nbudiku:\n%2lu Dni\n%2lu Hodin\n%2lu Minut\n%2lu Sekund", daysToAlarm, hrsToAlarm, minToAlarm, secToAlarm);
  } else {
    lv_label_set_text_static(txtMessage, "Budik\nneni\nnastaven.");
  }
}
...
void Alarm::SetRecurButtonState() {
  ...
  switch (alarmController.Recurrence()) {
    case AlarmController::RecurType::None:
      lv_label_set_text_static(txtRecur, "JEDNOU");
      break;
    case AlarmController::RecurType::Daily:
      lv_label_set_text_static(txtRecur, "DENNE");
      break;
    case AlarmController::RecurType::Weekdays:
      lv_label_set_text_static(txtRecur, "PO-PA");
  }
}
